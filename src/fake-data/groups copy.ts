//import { Groups } from "../types";

export default {
  escalada: [
    {
      name: "No seas psoe que quiero ir a la montaña",
      timestamp: 1579102200,
      place: "Buitrago de Lozoya",
      sizeGroup: 10,
      users: []
    }
  ],
  tirolinas: "Aún no hay grupos, crea uno",
  rafting: [
    {
      name: "Menos Madrid y más campo",
      timestamp: 1581957000,
      place: "Guadarrama",
      sizeGroup: 10,
      users: []
    },
    {
      name: "Podemos ir al río",
      timestamp: 1607783400,
      place: "Rascafría",
      sizeGroup: 10,
      users: []
    }
  ],
  piraguismo: "Aún no hay grupos, crea uno",
  moto_de_agua: [
    {
      name: "Ciudadanos caminando",
      timestamp: 1576236600,
      place: "San Martín de Valdeiglesias",
      sizeGroup: 10,
      users: []
    },
    {
      name: "Pepe y Pepa se van de juega",
      timestamp: 1577705400,
      place: "El Atazar",
      sizeGroup: 10,
      users: []
    }
  ],
  quad: "Aún no hay grupos, crea uno",
  senderismo: [
    {
      name: "Enanitos en el VoxQue",
      timestamp: 1598005800,
      place: "Navacerrada",
      sizeGroup: 10,
      users: []
    }
  ],
  ski: "Aún no hay grupos, crea uno",
  snowboard: "Aún no hay grupos, crea uno"
}; //as Groups;
